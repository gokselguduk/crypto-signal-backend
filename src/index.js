...
app.get('/api/candles/:symbol/:interval', function(req, res) {
  binance.getHistoricalCandles(req.params.symbol, req.params.interval, 200)
    .then(function(c) { res.json({ symbol: req.params.symbol, interval: req.params.interval, count: c.length, candles: c }); })
    .catch(function(e) { res.status(500).json({ error: e.message }); });
});

app.get('/api/analyze/:symbol/:interval', function(req, res) {
  binance.getHistoricalCandles(req.params.symbol, req.params.interval, 200)
    .then(function(c) { res.json(Object.assign({ symbol: req.params.symbol, interval: req.params.interval }, indicators.analyzeCandles(c))); })
    .catch(function(e) { res.status(500).json({ error: e.message }); });
});

app.get('/api/backtest/:symbol/:interval', function(req, res) {
  binance.getHistoricalCandles(req.params.symbol, req.params.interval, 1000)
    .then(function(c) {
...
app.get('/api/meme/radar', function(req, res) {
  ...
  memeRadar.scanMemeRadar()
    .then(function(data) { lastMemeData = data; res.json(data); })
    .catch(function(e) { res.status(500).json({ error: e.message }); });
});
...
app.get('/api/scan/latest', function(req, res) {
  res.json({ signals: scanner.getLastSignals() });
});
